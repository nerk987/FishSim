...
class RigUI(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        controls = ['head', 'neck', 'chest', 'hips', 'torso', 'spine', 'spine.001', 'spine.002', 'spine_master']
        torso    = 'torso'
        ...
        controls    = ['back_fin.B.Bk', 'back_fin.B.001.Bk', 'back_fin_masterBk']
        master_name = 'back_fin_masterBk'
        if is_selected(controls):
            layout.prop(pose_bones[master_name], '["finger_curve"]', text="Curvature", slider=True)
        

        
        controls    = ['back_fin.T.Bk', 'back_fin.T.001.Bk', 'back_fin.T.002.Bk', 'back_fin_masterBk.001']
        master_name = 'back_fin_masterBk.001'
        if is_selected(controls):
            layout.prop(pose_bones[master_name], '["finger_curve"]', text="Curvature", slider=True)
        

        
        controls    = ['pec_fin.R', 'pec_fin01.R', 'pec_fin02.R', 'pec__master.R']
        master_name = 'pec__master.R'
        if is_selected(controls):
            layout.prop(pose_bones[master_name], '["finger_curve"]', text="Curvature", slider=True)
        

        
        controls    = ['pec_fin.R.001', 'pec_fin01.R.001', 'pec_fin02.R.001', 'pec_fin._master01']
        master_name = 'pec_fin._master01'
        if is_selected(controls):
            layout.prop(pose_bones[master_name], '["finger_curve"]', text="Curvature", slider=True)
        

        
        controls    = ['pec_fin.L', 'pec_fin01.L', 'pec_fin02.L', 'pec__master.L']
        master_name = 'pec__master.L'
        if is_selected(controls):
            layout.prop(pose_bones[master_name], '["finger_curve"]', text="Curvature", slider=True)
        

        
        controls    = ['pec_fin.L.001', 'pec_fin01.L.001', 'pec_fin02.L.001', 'pec_fin._master01.001']
        master_name = 'pec_fin._master01.001'
        if is_selected(controls):
            layout.prop(pose_bones[master_name], '["finger_curve"]', text="Curvature", slider=True)
...
class RigLayers(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = col.row()
        row.prop(context.active_object.data, 'layers', index=1, toggle=True, text='Face (Tweak)')
        ...
        row = col.row()
        row.prop(context.active_object.data, 'layers', index=5, toggle=True, text='Tail')

        row = col.row()
        row.prop(context.active_object.data, 'layers', index=6, toggle=True, text='Fins.L')
        row.prop(context.active_object.data, 'layers', index=8, toggle=True, text='Fins.R')

        row = col.row()
        row.prop(context.active_object.data, 'layers', index=7, toggle=True, text='Fins.L (Tweak)')
        row.prop(context.active_object.data, 'layers', index=9, toggle=True, text='Fins.R (Tweak)')

        row = col.row()
        row.prop(context.active_object.data, 'layers', index=10, toggle=True, text='Fins')

        row = col.row()
        row.prop(context.active_object.data, 'layers', index=11, toggle=True, text='Fins (Tweak)')
